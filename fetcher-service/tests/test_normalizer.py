...
def test_normalize_reads_optional_precipitation_probability_max():
    data = build_open_meteo_response(days=11)

    normalized = normalize_open_meteo_response(data, make_settings())

    assert all(row.precipitation_probability_max == 10 for row in normalized.daily)


def test_normalize_tolerates_missing_precipitation_probability_max():
    data = build_open_meteo_response(days=11)
    del data["daily"]["precipitation_probability_max"]

    normalized = normalize_open_meteo_response(data, make_settings())

    assert all(row.precipitation_probability_max is None for row in normalized.daily)
