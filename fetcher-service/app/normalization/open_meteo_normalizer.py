...
def _optional_indexed(block: dict[str, Any], field: str, index: int) -> float | None:
    """Reads an optional daily field (present for every date Open-Meteo returns, but not
    required -- older API responses or a stripped-down fixture may omit it)."""
...
def _build_daily_row(block: dict[str, Any], index: int, tz: ZoneInfo) -> DailyWeather:
    try:
        return DailyWeather(
            ...
            precipitation_probability_max=_optional_indexed(block, "precipitation_probability_max", index),
