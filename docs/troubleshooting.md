...
**RabbitMQ queue stayed empty and the UI showed no weather after `vagrant up` — recurred on every plain VM restart, not just once (Stage 5)**
- Problem: `rabbitmqctl list_queues` on the `postgres` VM showed no messages ever arriving, `backend-worker.service` was crash-looping (`systemctl status` showing `activating (auto-restart)`), and `GET /api/weather` returned no data. Fixed once by deleting and regenerating `.env` on the affected VMs — then came back on its own after a later plain `vagrant up`, with no `.env` edits made in between.
- First (incomplete) theory: `.env` on VMs created *before* `RABBITMQ_URL` was added to `.env.example` would never get that line, since `[ -f .env ] || cp .env.example .env` only creates `.env` when it's missing, so the provisioning script's `sed` rewrite had nothing to match. This explained the first occurrence but not the recurrence, since the `.env` had already been regenerated correctly once.
- Actual root cause: `.env` is created only inside the guest (never present in the host repo — it's gitignored), and `Vagrantfile`'s rsync synced folder wasn't excluding it. Vagrant's rsync synced-folder type defaults to `--delete`, which mirrors the guest's `/app` to exactly match the host's `backend-service`/`fetcher-service` directory on every `vagrant up`/`reload` — anything guest-only that isn't in `RSYNC_EXCLUDES` gets deleted. `.venv` already survived this because it was excluded; `.env` wasn't, so it got wiped on every restart that wasn't a full `--provision` run, and since provisioners don't automatically re-run on a plain `up`/`reload`, the file was never regenerated on its own — both the Fetcher and the Backend Worker silently fell back to `app/config.py`'s compiled-in default, `amqp://guest:guest@localhost:5672/`, `localhost` from each VM's own perspective, where nothing is listening.
- Diagnosed by: `journalctl -u fetcher-service` showing `aiormq.connection: error when creating transport: <AMQPConnectionError: (111, 'Connection refused')>` against `amqp://guest:******@localhost:5672/`, alongside a *fresh* "Application startup complete" line — meaning the process had actually restarted (ruling out "it's just a stale process"); `grep RABBITMQ /app/.env` on both VMs printed nothing at all, confirming the lines were gone from the file itself, not just unread by a stale process; `backend-service` (the API) kept working throughout since its own process was never restarted and had the correct settings cached in memory (`get_settings()`'s `@lru_cache`) from before the wipe, even though the file on disk was already wrong — which is what made the UI's "no weather data" symptom show up before any other visible sign that something had regressed.
- Fix: added `.env` to `RSYNC_EXCLUDES` in the `Vagrantfile`, the same way `.venv` was already excluded — this stops Vagrant's rsync from ever touching `.env` again, so it survives every future `vagrant up`/`reload` untouched. Applying it to an already-broken environment still needs the one-time repair: `sudo rm -f /app/.env` on the affected VMs, `vagrant provision <name>` to regenerate it correctly, then `systemctl restart` the affected services (re-provisioning alone doesn't restart an already-running unit).
- Verify: `grep RABBITMQ /app/.env` on both VMs shows the LAN address, not `localhost`; `rabbitmqctl list_queues name messages consumers` shows `consumers = 1` on both queues; critically, this now needs to be re-verified *after a plain `vagrant up`/`reload`* (not just after a `vagrant provision`) to confirm the fix actually holds across a restart, since that's exactly the scenario the original fix didn't cover.
- Avoid later: any file a provisioning script generates *inside* a synced folder (not just `.env` — anything written to `/app` by a `provision.sh` that doesn't also exist in the host repo) needs to be in `RSYNC_EXCLUDES`, or Vagrant's default `--delete` rsync behavior will silently erase it on every subsequent sync. This is easy to miss because the symptom shows up as an application-level config regression (wrong URL, crash-looping service), not as an obviously infrastructure-shaped error.
