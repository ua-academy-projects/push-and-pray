...
### The managed database's password

With `database.mode` set to `managed`, the `POSTGRES_PASSWORD` container gets
its value differently on each cloud, and the `managed_database_credentials`
playbook is what reconciles it — see `docs/managed-database.md`. On AWS the
value is generated by RDS and copied into the container by that playbook, so
there is nothing to export for it. On GCP the container is uploaded as usual,
and the playbook creates the application role on the instance with the same
value — from the environment variable, or from the container's latest version
when the operator can read it.

`RABBITMQ_PASSWORD` and `REDIS_PASSWORD` are ordinary containers, uploaded like
any other. They exist in both modes, because the mappings are static; only the
managed mode reads them.
