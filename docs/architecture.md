...
**Refactor complete (Stage 5 of 5), plus two later simplifications (decisions 30–31).** SkyIvano was originally built as three services (UI, Backend, History) and has been restructured into three application services (UI, Backend, a Weather Fetcher) plus PostgreSQL, Redis, and RabbitMQ. Stage 1 merged the old History Service's persistence layer into the Backend. Stage 2 extracted all Open-Meteo/scheduling responsibility out of the Backend into a new Weather Fetcher Service, connected to the Backend by a synchronous internal HTTP contract. Stage 3 added the statistics/forecast API the UI needs. Stage 4 rebuilt the UI to consume all of it — see §15. Stage 5 replaced that Stage 2 HTTP contract with asynchronous messaging over RabbitMQ: the Fetcher publishes a completed (or failed) sync instead of pushing it over HTTP, initially consumed and persisted by a separate Backend Worker process. Decision 30 later merged that consumer back into the Backend's own FastAPI process as a background task, and decision 31 replaced Alembic migrations with `Base.metadata.create_all()` on startup — see §5, §7, §8, §9, §17. The History Service no longer exists in the repository at all — see `docs/prompts/refactor-0-discovery-and-plan.md` for the full staged plan.

Three independently runnable application processes (one per service) plus PostgreSQL, Redis, and RabbitMQ:

- **UI Service** — React/TypeScript dashboard. Reads only.
- **Backend Service** — FastAPI. Owns PostgreSQL directly: serves the public API to the UI, and — as a background task inside the same process (§17, decision 30) — consumes synced weather data from RabbitMQ and persists it via `persistence_service`, the same code the API used to call synchronously before Stage 5. Also owns Redis, for ephemeral UI session state only (§16) — never calls Open-Meteo.
- **Weather Fetcher Service** — FastAPI. The only service that calls Open-Meteo. Runs the sync scheduler independently of user traffic, normalizes the response, and publishes it to RabbitMQ for the Backend to consume. Never touches PostgreSQL, never touches Redis, never pushes data to the Backend over HTTP anymore.
...
- **RabbitMQ** — the durable queue carrying completed/failed syncs from the Fetcher to the Backend (§17). Co-located on the same `postgres` VM (§14, §17).
...
flowchart TD
    ...
    RabbitMQ -->|consume| Backend[Backend Service]
...
| Backend | Read queries, freshness calculation, public API, proxying one manual-refresh call to the Fetcher, session state (Redis), plus (as a background task in the same process, §17, decision 30) consuming `weather.sync`/`weather.sync_failure` from RabbitMQ and persisting via `persistence_service` | Call Open-Meteo, run a scheduler, contain SQLAlchemy-free normalization logic |
...
sequenceDiagram
    ...
    participant W as Backend (consumer task)
...
**"Backend (consumer task)" is not a separate process** — as of decision 30, it's a background `asyncio` task running inside the same FastAPI process as the API itself (see §17), shown as its own lane here only because it's logically a distinct, concurrently-running piece of code.
...
**As of Stage 5, this flow's HTTP response no longer means "persisted."** Through Stage 2–4, `SyncTriggerResult` reflected a completed database write, because the whole chain was synchronous. Now the Fetcher returns to the Backend as soon as it has published to RabbitMQ — the Backend's background consumer task (§17, decision 30) actually upserts afterward, out of band (§5, §17). A `"success"` status here means "Open-Meteo was called and the result was queued," not "the dashboard already reflects it." The UI's immediate re-fetch of `/api/weather` right after this may still show the *previous* data; the next poll tick (`useWeather`'s 60-second interval, §15) is what actually picks up that write once it lands. This was a deliberate trade-off: a "queued" response accurately describes what the Fetcher can promise once persistence is decoupled from it, rather than the Backend blocking the HTTP response on a RabbitMQ consumer it doesn't control.
...
| `POST /api/sync/trigger` | The one deliberate exception to "Backend never calls Fetcher" — proxies to the Fetcher's `POST /internal/fetch` and relays the result. Never returns Open-Meteo's raw response or a stack trace. **As of Stage 5, a `"success"` status means the fetch succeeded and was published to RabbitMQ — not that the Backend's background consumer task has persisted it yet** (§7, §17). |
...
Owned exclusively by the Backend Service. SQLAlchemy 2 models; tables are created via `Base.metadata.create_all()` on startup (`app/database/session.py`'s `init_db()`) rather than migration tooling — see decision 31. Sync SQLAlchemy (`psycopg` v3 driver) is used deliberately over async — this is a low-throughput, single-writer service, and sync SQLAlchemy keeps sessions and transactions simple. The Backend's few outbound HTTP calls (`POST /api/sync/trigger`'s Fetcher proxy) stay async (httpx); sync DB work is never mixed into that same coroutine without going through `asyncio.to_thread()` or a request-scoped thread-pooled route.
...
- Originally backfilled for pre-Stage-3 rows by an Alembic data migration that joined existing `hourly_weather` rows by location + local calendar date (converting hourly's UTC storage via `locations.timezone`), falling back to `(min+max)/2` for any day with no matching hourly data. That migration (and Alembic itself) no longer exists in the repository as of decision 31 — it was a one-time backfill against data that already existed at the time, not an ongoing schema-management need.
...
`open_meteo_request_url`, `open_meteo_status_code`, and `open_meteo_duration_ms` record the actual outbound call to Open-Meteo (the only external API this system calls, and now only the Fetcher makes it) — not just whether the resulting sync succeeded, but what was requested, what HTTP status came back, and how long it took. The Fetcher measures these around its `open_meteo_client` call and forwards them to the Backend as part of the `weather.sync` message (success case) or `weather.sync_failure` message (failure case, where `open_meteo_status_code`/`open_meteo_duration_ms` may be null if the call never returned, e.g. a timeout). All three columns are nullable for `skipped` syncs, which never call Open-Meteo at all.
...
- Previously persisted data is never deleted or partially overwritten — `persistence_service.upsert_weather` runs in one transaction; any failure rolls the whole thing back, unchanged since Stage 1. A `PersistenceError` from that rollback used to be caught by an explicit FastAPI exception handler and returned as a clean `500` when this ran inside `PUT /internal/weather/sync`; as of Stage 5 it's raised inside the RabbitMQ message handler instead (`app/broker/consumer.py`, running as a background task in the same process as of decision 30), where it causes that message to be nacked and dead-lettered rather than turned into an HTTP response (§17) — either way, the rollback guarantee itself is identical.
...
- RabbitMQ itself has no meaningful access control in this assignment beyond a non-`guest` username/password (required anyway once the Fetcher and Backend are on different LAN hosts — RabbitMQ refuses `guest` from outside `localhost`). No TLS on the AMQP connection, no per-queue permission scoping beyond the one shared user. Production should add TLS and a narrower-permissioned user per service.
...
29. **A message that can't be processed is dead-lettered once, not retried indefinitely** — both queues declare `x-dead-letter-exchange`/`x-dead-letter-routing-key` pointing at a shared `weather.dlx` exchange, routing a nacked message to `<queue>.dead`. The Backend's consumer (`app/broker/consumer.py`) always nacks with `requeue=False` on any failure — a bad message shape or a rolled-back persistence error will not succeed on a bare retry, so looping it back onto the same queue forever would just be a slow-motion outage; landing it once on the dead-letter queue for manual inspection was judged simpler than building retry-count tracking (e.g. inspecting `x-death` headers) for a local, single-instance assignment. `persistence_service`'s upserts being idempotent (decision 8) means the one case that *is* safely retryable — redelivery after the consuming process crashes mid-write, before acking — is already handled by RabbitMQ's own redelivery-on-disconnect behavior, without needing the dead-letter path at all.

30. **The RabbitMQ consumer was merged back into the Backend's own FastAPI process as a background `asyncio` task, reversing decision 27** — running it as a separate `worker` container turned out to cost more in operational complexity (a second container, a second set of resource limits/healthcheck rules, a second thing to reason about per deploy) than the isolation was worth for this project's actual scale and traffic. The consumer (`app/broker/consumer.py`'s `consume_forever`) now starts as an `asyncio.create_task` in `app/main.py`'s FastAPI `lifespan`, sharing the same event loop as the HTTP server. The one deliberate safeguard kept from the old design: if the consumer task ever raises (lost connection, unhandled error), a `done_callback` calls `os._exit(1)` rather than letting it die silently while the API keeps looking healthy — gunicorn (pinned to `--workers 1`, so exactly one consumer instance ever runs) notices the worker process died and respawns a clean one, approximating the old container-level `restart: unless-stopped` behavior without a second container. `backend-service/docker-compose.yml` is back down to a single `api` service (§18).
31. **Alembic migrations were replaced with `Base.metadata.create_all()` run at startup** — `app/database/session.py`'s `init_db()` creates any tables that don't exist yet, from the current model definitions, every time the Backend starts; it's a no-op once every table already exists. This trades away Alembic's real capabilities — altering an existing table's columns in place, and one-time data backfills against already-existing rows (like the `average_*` columns' backfill, §9) — for a much simpler setup with no separate migration step or file history to maintain. Accepted for this project's stage: schema changes to a table that already has real data would now require recreating the database or a hand-written one-off script, rather than a tracked, repeatable migration — a real limitation if this project later needs to evolve a live, populated database in place.
...
|---|---|---|---|
| `weather.sync` | Fetcher, on a successful fetch+normalize (`RabbitMQPublisher.publish_weather_sync`) | Backend, in its background consumer task (`handle_weather_sync_message`) | Same shape as the old `WeatherUpsertRequest`: location, current, daily, daily_forecast, hourly, source, sync metadata |
| `weather.sync_failure` | Fetcher, when Open-Meteo or normalization failed (`RabbitMQPublisher.publish_sync_failure`) | Backend, in its background consumer task (`handle_sync_failure_message`) | Same shape as the old `SyncFailureRequest`: location, trigger_type, timestamps, error_message, Open-Meteo call metadata |
...
**Delivery guarantee: at-least-once, made safe by idempotent upserts.** The consumer acknowledges a message only after `persistence_service.upsert_weather`/`record_sync_failure` has committed (`app/broker/consumer.py`'s `message.process()` block) — not on receipt. If the Backend process crashes or disconnects between receiving a message and acking it, RabbitMQ redelivers it to the next available consumer. Since every persistence write here is an upsert on a stable unique key (§9), replaying the same message twice is harmless — this is the same reasoning that already made the Stage 2 HTTP contract safe to retry, just applied to redelivery instead of an HTTP retry.
...
| `app/main.py` | As of decision 30, starts `consume_forever()` as an `asyncio.create_task` in its FastAPI `lifespan`, alongside `uvicorn`'s own event loop — no separate entry point or OS process (there is no `app/worker.py` anymore) |
...
**backend VM (`backend-service/docker-compose.yml`)** — a single `api` container (decisions 30–31 reversed the original three-container `migrate`/`api`/`worker` split described in earlier revisions of this doc). Schema setup is not a separate step: `init_db()` creates any missing tables on startup, so there's no `migrate` container to run first or wait on. The RabbitMQ consumer isn't a separate `worker` container either — it runs as a background task inside the same process serving the API (§17, decision 30). Gunicorn is pinned to `--workers 1` specifically so exactly one consumer instance ever runs; more workers would each start their own independent consumer, competing for the same queues.
