...
| the bastion's `ssh_port`, defaulting to `22` | the bastion's `ansible_port` |
...
`ProxyCommand` generated by `oilscope.platform.oilscope`. Inventory selects a
...
The plugin owns `oilscope_ssh_base_args` and `ansible_ssh_common_args` for each
discovered host; no static `group_vars` or `host_vars` are required.
Both the target connection and the bastion jump use
`StrictHostKeyChecking=accept-new` with a `HostKeyAlias` scoped to the cloud,
GCP project or AWS region, and immutable instance ID. Recreated VMs therefore
get new known-host entries even when their IP is reused. Changed keys for the
same instance are still rejected. Existing IP-based entries are retained;
the first connection after this migration records the instance-based entry.
Refresh the inventory cache after replacing a VM if it still reports the old ID.
...
| SSH times out directly on a workload's private IP, without a bastion jump | the installed `oilscope.platform` collection may be stale; rebuild and reinstall it using the Setup commands above, then verify `ansible_ssh_common_args` in the generated inventory |
