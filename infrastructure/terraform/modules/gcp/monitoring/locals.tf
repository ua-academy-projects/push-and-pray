locals {
  ...
  budget_enabled     = var.config.budgets.gcp.enabled
  ...
  synthetics_enabled = local.settings.enabled && local.settings.synthetics.enabled && (local.settings.synthetics.clouds == null ? length(local.workload_vms) > 0 : contains(local.settings.synthetics.clouds, "gcp"))
  recipients         = local.enabled ? local.settings.email_recipients : toset([])
  vm_filters         = { for name, vm in local.workload_vms : name => "resource.type=\"gce_instance\" AND resource.labels.project_id=\"${local.project_id}\" AND resource.labels.instance_id=\"${vm.instance_id}\" AND resource.labels.zone=\"${vm.zone}\"" }
  ...
  vm_signals = merge({}, [for name, vm in local.workload_vms : { for signal, definition in local.signals : "${name}-${signal}" => merge(definition, { name = name, signal = signal, filter = "${local.vm_filters[name]} AND metric.type=\"${definition.metric}\"${definition.extra}" }) }]...)
  absence_signals = merge({}, [for name, vm in local.workload_vms : merge({
