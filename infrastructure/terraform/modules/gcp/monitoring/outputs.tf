...
output "alert_policy_ids" { value = merge({ for k, v in google_monitoring_alert_policy.vm : k => v.name }, { for k, v in google_monitoring_alert_policy.missing : k => v.name }, { for k, v in google_monitoring_alert_policy.http : k => v.name }, { for k, v in google_monitoring_alert_policy.uptime : "uptime-${k}" => v.name }) }
