...
output "alarm_arns" {
  value = merge({ for k, v in aws_cloudwatch_metric_alarm.vm : k => v.arn }, { for k, v in aws_cloudwatch_metric_alarm.http : k => v.arn }, { for k, v in aws_cloudwatch_metric_alarm.synthetic : "synthetic-${k}" => v.arn })
}
