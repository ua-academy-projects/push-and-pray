...
variable "database_managed" {
  description = "Whether PostgreSQL is a managed service. Decides what the infra group admits from the workloads: the PostgreSQL port when the VM runs the database itself, the broker and cache ports when the database lives elsewhere."
  type        = bool
  default     = false
}

variable "vpc_id" {
  description = "ID of the VPC these security groups live in. The only thing this module needs from the network."
  type        = string
}

variable "tags" {
  description = "Tags applied to every security group and rule."
  type        = map(string)
}
