...
from .clients import CoinGeckoClient, FrankfurterClient
...
class RateService:
    def __init__(self, crypto: CoinGeckoClient, fiat: FrankfurterClient):
        self.crypto = crypto
        self.fiat = fiat
        self.cache = TTLCache()
    ...
    async def backfill(self, instrument_id: str, start: date, end: date) -> list[Rate]:
        kind, base, quote = self.parse(instrument_id)
        series = await self.provider_history(instrument_id, start, end)
        market_caps: dict[datetime, Decimal] = {}
        if kind == "crypto":
            days = max(1, min(365, (end - start).days))
            market_caps = {
                point.timestamp: point.value
                for point in await self.crypto.market_cap_history(base, quote, days)
            }
        ...
        rates = [Rate(
            ...
            price=point.value, market_cap=market_caps.get(point.timestamp),
            source=series.source, source_timestamp=point.timestamp,
