...
from history_service.logging_config import UvicornFields, configure_logging
...
def test_other_records_pass_through() -> None:
    record = logging.LogRecord("history_service", logging.INFO, __file__, 0, "hi", (), None)
...
def test_lines_are_json_with_level_and_message(captured_stdout: io.StringIO) -> None:
    logging.getLogger("history_service.test").warning("queue %s is slow", "price_observations")

    entry = json.loads(captured_stdout.getvalue().strip())

    assert entry["level"] == "WARNING"
    assert entry["logger"] == "history_service.test"
