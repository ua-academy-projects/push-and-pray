...
class RabbitConsumer:
    """RabbitMQ implementation of the history consumer, used in managed mode.

    Mirrors PGMQConsumer's contract (start/stop/is_ready) so main.py can pick
    either backend by QUEUE_BACKEND. Persistence stays idempotent through the
    same insert_batch (unique instrument_code + scheduled_for), so a redelivery
    is harmless — dedup does not depend on the pgmq-only published_queue_events
    table.
    """
