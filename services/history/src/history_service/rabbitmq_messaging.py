...
class RabbitMQConsumer:
    ...
    async def start(self) -> None:
        ...
        dead_letter_exchange = await self.channel.declare_exchange(
            f"{self.settings.pgmq_queue}.dead-letter", durable=True
        )
        dead_letter_queue = await self.channel.declare_queue(
            f"{self.settings.pgmq_queue}.dead-letter", durable=True
        )
        await dead_letter_queue.bind(dead_letter_exchange, routing_key=self.settings.pgmq_queue)

        queue = await self.channel.declare_queue(
            self.settings.pgmq_queue,
            durable=True,
