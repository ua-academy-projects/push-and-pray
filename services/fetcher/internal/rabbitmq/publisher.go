...
func (publisher Publisher) publishOnce(ctx context.Context, eventKey string, body []byte) error {
	...
	if err := channel.ExchangeDeclare(publisher.Exchange, "direct", true, false, false, false, nil); err != nil {
		return fmt.Errorf("declare exchange: %w", err)
	}
	deadExchange := publisher.Exchange + ".dead-letter"
	if err := channel.ExchangeDeclare(deadExchange, "direct", true, false, false, false, nil); err != nil {
		return fmt.Errorf("declare dead-letter exchange: %w", err)
	}
	deadQueue := publisher.Queue + ".dead-letter"
	if _, err := channel.QueueDeclare(deadQueue, true, false, false, false, nil); err != nil {
		return fmt.Errorf("declare dead-letter queue: %w", err)
	}
	if err := channel.QueueBind(deadQueue, publisher.RoutingKey, deadExchange, false, nil); err != nil {
	...
	if err := channel.QueueBind(publisher.Queue, publisher.RoutingKey, publisher.Exchange, false, nil); err != nil {
	...
	if err := channel.PublishWithContext(ctx, publisher.Exchange, publisher.RoutingKey, true, false, amqp.Publishing{
