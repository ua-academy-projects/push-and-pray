import { formatMoney, percentSeries } from './utils.js?v=11';
...
const state = { instruments: [], primary: null, comparisons: [], selected: new Set(), period: '1d', step: '5m', mode: 'price', graphs: [], marketPeriod: '1d', marketMaps: [] };
...
function initTheme() {
  ...
  button.addEventListener('click', () => { const theme = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark'; document.documentElement.dataset.theme = theme; localStorage.setItem('rateboard-theme', theme); sync(); }); sync();
}
...
function renderHero() {
  ...
  if (state.primary) { primaryCard.classList.remove('skeleton'); primaryCard.innerHTML = rateCard(state.primary, true, 'primary', true); primaryCard.querySelector('.remove-card').addEventListener('click', () => { state.primary = null; renderHero(); }); }
  ...
  document.querySelectorAll('.comparison-rate .remove-card').forEach(button => button.addEventListener('click', () => { state.comparisons.splice(Number(button.dataset.index), 1); renderHero(); }));
}
...
async function selectPrimary(id) { try { const { items } = await api(`/rates/current?instruments=${encodeURIComponent(id)}`); state.primary = await hydrateWeekly(items[0]); renderHero(); scrollTo({ top: 0, behavior: 'smooth' }); } catch (error) { showError(error.message); } }
...
async function loadOverview() {
  try {
    ...
    await hydrateWeekly(state.primary); bindRows($('#crypto-list')); bindRows($('#fiat-list')); renderHero(); renderInstrumentOptions(); renderPicker();
  } catch (error) { showError(`${error.message}. Переконайтеся, що Backend запущений.`); $('#primary-card').innerHTML = '<p class="empty">Дані тимчасово недоступні.</p>'; }
}
...
function renderInstrumentOptions(query = '') {
  ...
  $('#instrument-options').innerHTML = matches.map(item => `<label class="instrument-option"><input type="checkbox" value="${item.instrument_id}" ${state.selected.has(item.instrument_id) ? 'checked' : ''}><span><strong>${item.name}</strong><br><small>${item.base}/${item.quote}</small></span></label>`).join('');
  $('#instrument-options').querySelectorAll('input').forEach(input => input.addEventListener('change', () => { if (input.checked && state.selected.size >= 5) { input.checked = false; showError('Можна обрати не більше п’яти валют.'); return; } input.checked ? state.selected.add(input.value) : state.selected.delete(input.value); syncSelection(); })); syncSelection();
}
function syncSelection() { $('#selection-count').textContent = state.selected.size; $('#instrument-trigger').textContent = state.selected.size ? [...state.selected].map(id => state.instruments.find(x => x.instrument_id === id)?.base).join(', ') : 'Оберіть до 5 валют'; if (state.selected.size) showError(); }
...
async function fetchGraph(config) {
  ...
  return data.series.map(series => ({ ...series, points: series.points.filter(point => new Date(point.timestamp) >= start) }));
}
...
function drawGraph(graph, series) {
  ...
  const colors = ['#168a4b', '#3478c7', '#e28b25', '#9b63ce', '#d1495b'];
  const datasets = series.map((item, index) => { const points = graph.mode === 'percent' ? percentSeries(item.points) : item.points; return { label: item.instrument_id.split(':').slice(1).join('/').toUpperCase(), data: points.map(point => ({ x: new Date(point.timestamp).getTime(), y: Number(point.value) })), borderColor: colors[index], backgroundColor: colors[index], pointRadius: 0, pointHoverRadius: 4, borderWidth: 2, tension: 0 }; });
...
function removeGraph(id) { const index = state.graphs.findIndex(x => x.id === id); if (index >= 0) { state.graphs[index].chart?.destroy(); state.graphs.splice(index, 1); } document.querySelector(`[data-graph-id="${id}"]`)?.remove(); }
...
function resetGraphs() { state.graphs.forEach(graph => graph.chart?.destroy()); state.graphs = []; $('#charts-container').replaceChildren(); }
...
async function addMarketMap() {
  ...
  try { const data = await api(`/market-map?period=${state.marketPeriod}`); const map = { id: crypto.randomUUID(), period: state.marketPeriod, items: data.items }; state.marketMaps.push(map); marketMapCard(map); }
  catch (error) { showError(error.message); } finally { button.disabled = false; }
}
...
function buildPDFReport() {
  ...
  $('#pdf-report').innerHTML = `<header><p class="eyebrow">Rateboard</p><h1>Дані на сьогоднішній день</h1><p class="pdf-generated">Сформовано: ${generated}</p></header><section><h2>Детальна інформація з карток</h2>${cardContent}</section><section><h2>Карти капіталізації</h2>${marketContent}</section><section><h2>Графіки</h2>${graphContent}</section><footer class="pdf-report-footer"><span>© Rateboard 2026</span><span>Зроблено за допомогою сервісу Rateboard</span></footer>`;
}
function exportPDF() { buildPDFReport(); window.print(); }

function initEvents() {
  document.querySelectorAll('.tab').forEach(tab => tab.addEventListener('click', () => { document.querySelectorAll('.tab').forEach(item => { const active = item === tab; item.classList.toggle('active', active); item.setAttribute('aria-selected', active); }); $('#overview-panel').hidden = tab.dataset.tab !== 'overview'; $('#history-panel').hidden = tab.dataset.tab !== 'history'; $('#market-map-panel').hidden = tab.dataset.tab !== 'market-map'; }));
  ...
  $('#period-buttons').addEventListener('click', event => { if (!event.target.dataset.period) return; state.period = event.target.dataset.period; [...event.currentTarget.children].forEach(x => x.classList.toggle('active', x === event.target)); });
  $('#step-buttons').addEventListener('click', event => { if (!event.target.dataset.step) return; state.step = event.target.dataset.step; [...event.currentTarget.children].forEach(x => x.classList.toggle('active', x === event.target)); });
  $('#mode-buttons').addEventListener('click', event => { if (!event.target.dataset.mode) return; state.mode = event.target.dataset.mode; [...event.currentTarget.children].forEach(x => x.classList.toggle('active', x === event.target)); });
  ...
  $('#market-period-buttons').addEventListener('click', event => { if (!event.target.dataset.period) return; state.marketPeriod = event.target.dataset.period; [...event.currentTarget.children].forEach(item => item.classList.toggle('active', item === event.target)); }); $('#add-market-map').addEventListener('click', addMarketMap); $('#refresh-market-maps').addEventListener('click', refreshMarketMaps); $('#reset-market-maps').addEventListener('click', resetMarketMaps);
}

initTheme(); initEvents(); scheduleGraphRefresh(); loadOverview();
